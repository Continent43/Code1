...
void mousePressed() {
  if (dist(mouseX, mouseY, width/4, height/4) < 50 && mousePressed ) {
    on1 = true;
  }else {
    on1 = false;
  }
  if (dist(mouseX, mouseY, width/4, 3 * height/4) < 50 && mousePressed) {
    on2 = true;
  }else {
    on2 = false;
  }
  if (dist(mouseX, mouseY, 3 * width/4, height/4) < 50 && mousePressed) {
    on3 = true;
  }else {
    on3 = false;
  }
  if (dist(mouseX, mouseY, 3 * width/4, 3 * height/4) < 50 && mousePressed) {
    on4 = true;
  }else {
    on4 = false;
  }
