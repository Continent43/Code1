...
float[][] ySpeeds = new float [gridW][gridH];
float[][] xSpeeds = new float [gridW] [gridH];
...
void setup() {
  ...
  for (int i = 0; i < gridW; i++) {
    for (int j = 0; j < gridH; j++) {
      ...
      xSpeeds[i][j] = random(-5, 5);
      ySpeeds[i][j] = random(-5, 5);
    }
  }
}
...
void draw() {
  ...
  for (int i = 0; i < gridW; i++) {
    for (int j = 0; j < gridH; j++) {
      float s = 15;
      xPositions[i][j] += xSpeeds[i][j];
      yPositions[i][j] += ySpeeds[i][j];
